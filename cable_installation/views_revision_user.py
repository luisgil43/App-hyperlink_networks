...
def _requirement_measurement_state(req):
    has_start = req.start_ft is not None
    has_end = req.end_ft is not None
...
def _row_has_any_started_data(row):
    req = row.requirement

    measurement_state = _requirement_measurement_state(req)
...
def _row_requires_measurement_resubmission(row):
    ...
    reviewed_at = getattr(assignment, "supervisor_revisado_en", None)
    if not reviewed_at:
        return False

    last_touch = _row_last_measurement_touch_at(row)
    if not last_touch:
        return True

    return last_touch <= reviewed_at
...
def _row_is_complete(row):
    ...
    started = _row_has_any_started_data(row)
    needs_measurement_resubmission = _row_requires_measurement_resubmission(row)

    if req.required:
        return (
            measurement_state == "complete"
            and not needs_measurement_resubmission
            and not pending_shots
            and not has_rejected
        )

    # Optional:
    # - untouched => no cuenta como completo, pero tampoco debe bloquear
    # - complete measurement => completo aunque no tenga fotos
    # - partial measurement or rejected => pendiente
    # - rejected supervisor general => debe volver a guardar medida
    if not started:
        return False
...
def _refresh_row_status(row):
    ...
    # Si existe rechazo individual de foto, ese comentario manda.
    # Si no existe, conservamos la observación general ya guardada.
    supervisor_note = latest_rejection or (row.supervisor_note or "")
...
def _faltantes_global_labels(billing):
    ...
    requirements = billing.cable_requirements.all().order_by(
    ...
    for req in requirements:
        ...
        started = _row_has_any_started_data(sample_row)
        needs_measurement_resubmission = _row_requires_measurement_resubmission(
            sample_row
        )

        if req.required:
            missing_measurement = (
                measurement_state != "complete" or needs_measurement_resubmission
            )
            missing_shots = [
        ...
        # Optional:
        # - untouched => no mostrar / no bloquear
        if not started:
            continue

        # - complete measurement and no rejected => no bloquear
        # - si hubo rechazo general, debe volver a guardar medida
        if (
            measurement_state == "complete"
            and not needs_measurement_resubmission
            and not has_rejected
        ...
        suffix = []
        if measurement_state != "complete" or needs_measurement_resubmission:
            suffix.append("measurement")
        if has_rejected:
            suffix.append("review")
...
def technician_requirements(request, assignment_id):
    ...
    for req in requirements:
        ...
        measurement_state = _requirement_measurement_state(req)
        partial_measurement_started = measurement_state == "partial"
        ...
        blocks.append(
            {
                ...
                "partial_measurement_started": partial_measurement_started,
            }
        )
