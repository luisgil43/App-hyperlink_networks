...
from .models import CustomUser, Rol, get_role_label_en


# -------- Field para mostrar los roles en INGLÉS en el checkbox --------
class RoleMultipleChoiceField(forms.ModelMultipleChoiceField):
    def label_from_instance(self, obj):
        # obj es una instancia de Rol
        return get_role_label_en(obj.nombre)
...
# -------- Form que usas en tu panel (crear/editar usuario) --------
class UsuarioForm(forms.ModelForm):
    ...
    # Roles (ManyToMany) requerido, MOSTRADOS EN INGLÉS
    ...
    def clean(self):
        ...
        if not (is_active or is_staff or is_super):
            ...
            self.add_error(
                'is_active',
                'Please choose at least one status (Active, Staff, or Superuser).',
            )
