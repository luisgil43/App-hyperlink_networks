...
def enqueue_cable_photo_report(job_id: int):
    ...
    def _run():
        ...
        try:
            from cable_installation.views import (
            ...
            ev_qs = _cable_report_evidences_qs(billing)
            total = ev_qs.count()
            ...
            def should_cancel(_done):
                fresh = ReporteFotograficoJob.objects.get(pk=job.pk)
                return bool(getattr(fresh, "cancel_requested", False))
            ...
            job.terminado_en = timezone.now()
            job.resultado_key = (
                getattr(billing.reporte_fotografico, "name", "") or key_name
            )
            job.procesadas = job.total
            job.save(
